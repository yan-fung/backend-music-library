...
describe('Update Albums', () => {
  ...
  beforeEach(async () => {
    ...
    const response = await db.query(
      `INSERT INTO Albums (album_name, year, artistId) VALUES ($1, $2, $3) RETURNING *`,
  ...
  describe('PUT /albums/{id}', () => {
    it('replaces the albums and returns the updated record', async () => {
      const { status, body } = await request(app)
        .put(`/albums/${album.id}`)
        .send({ album_name: 'something different', year: 1984, artistId: 1 });
      ...
      expect(body).to.deep.equal({
        id: album.id,
        album_name: 'something different',
        year: 1984,
        artistid: 1,
      });
    });
  ...
  describe('PATCH /albums/{id}', () => {
    it('updates the album and returns the updated record', async () => {
      const { status, body } = await request(app)
        ...
        .send({ album_name: 'Enchanted', year: 1984, artistId: 1 });
    ...
    it('returns a 404 if the artist does not exist', async () => {
      const { status, body } = await request(app)
        ...
        .send({ album_name: 'something different' });
